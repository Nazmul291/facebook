...
function InputBox({currentUser}) {
    ...
    if (docId){
        console.log(docId)
        setDoc(doc(db, 'posts', docId), { postImage: imagetoPost}, {merge:true}).catch(error => {
            console.log(error);
        })
  ...
  return (
    <div className='bg-white p-2 relative rounded-2xl shadow-md text-gray-500 font-medium mt-6'>
        ...
        {isLoading? <div className='h-full flex items-center flex-col justify-center w-full top-0 left-0 bg-white p-2 rounded-2xl shadow-md text-gray-500 absolute z-50 font-medium '>
            
            <Oval color="#00BFFF" className='flex mx-auto' height={80} width={80} />
        ...
        <div className='flex space-x-4 p-4 items-center '>
            ...
            {imagetoPost && (
                <div onClick={removeImage} className='flex flex-col cursor-pointer filter hover:brightness-110 transition duration-150 transform hover:scale-105 ' >
                    <img className='h-10 object-contain' src={preview} alt='preview post media' />
